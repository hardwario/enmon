...
static int i2c_reset(hid_device *handle)
{
    unsigned char data[] = { 0xa1, 0x20 };

    if (hid_send_feature_report(handle, data, sizeof(data)) != sizeof(data))
        return -1;

    return 0;
}

static int i2c_get_status(hid_device *handle, unsigned char **status)
{
...
static int set_led(hid_device *handle, int state)
{
...
static int set_system_clock(hid_device *handle)
{
...
static int set_uart_mode(hid_device *handle)
{
...
static int set_i2c_mode(hid_device *handle)
{
...
static int set_i2c_clock_speed(hid_device *handle)
{
...
static int get_system_status(hid_device *handle, unsigned char **status)
{
...
static int i2c_write(hid_device *handle, unsigned char address, const unsigned char *buffer, size_t length, int stop)
{
...
static int i2c_select(hid_device *handle, int bus)
{
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    if (set_system_clock(handle) != 0)
        die("Call `set_system_clock` failed");

    if (set_i2c_mode(handle) != 0)
        die("Call `set_i2c_mode` failed");

    if (set_uart_mode(handle) != 0)
        die("Call `set_uart_mode` failed");

    if (set_led(handle, 1) != 0)
        die("Call `set_led` failed");

    delay(100);

    if (set_led(handle, 0) != 0)
        die("Call `set_led` failed");

    if (i2c_reset(handle) != 0)
        die("Call `i2c_reset` failed");

    if (set_i2c_clock_speed(handle) != 0)
        die("Call `i2c_set_speed` failed");

    unsigned char *status;

    if (get_system_status(handle, &status) != 0)
